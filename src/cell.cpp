...
void Cell::add_sense()
{
    this->__sense_others += 1;
}

void Cell::forget_sense()
{
    this->__sense_others = 0;
}
...
void Cell::update()
{
    ...
    ins.push_back(static_cast<double>((int)(this->_coords.x - this->_field_margin) - 
        (int)(this->_field_size.x / 2)) / static_cast<double>(this->_field_size.x / 2));
    ins.push_back(static_cast<double>((int)(this->_coords.y - this->_field_margin) - 
        (int)(this->_field_size.y / 2)) / static_cast<double>(this->_field_size.y / 2));
    ins.push_back(
        static_cast<double>(this->__time_alive) / 600.
    );
    ins.push_back(
        sin((static_cast<double>(this->__time_alive) / 600.) * PI)
    );
    if (this->__sense_others > 10)
        this->__sense_others = 10;
    ins.push_back(
        static_cast<double>(this->__sense_others) / 10.
    );
    ins.push_back(
        static_cast<double>(this->__bumped) / 600.
    );
